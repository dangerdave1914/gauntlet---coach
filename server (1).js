...
/**
 * Apps Script answers a POST with a 302 to googleusercontent. Node's fetch
 * follows redirects but downgrades POST to GET per spec, which drops the body
 * and loses the request. So follow it by hand and re-POST.
 */
async function rpc(payload) {
  if (!SCRIPT_URL) throw new Error('APPS_SCRIPT_URL is not set');
  const body = JSON.stringify({ ...payload, secret: SCRIPT_SECRET });
  const opts = {
    method: 'POST',
  ...
  let res = await fetch(SCRIPT_URL, opts);
  for (let hop = 0; hop < 3 && res.status >= 300 && res.status < 400; hop++) {
    const next = res.headers.get('location');
    if (!next) break;
    res = await fetch(next, opts);
  }
  ...
  try {
    ...
    // Almost always the Google sign-in page: the deployment isn't set to
    // "Anyone", or a code edit was never redeployed as a new version.
    throw new Error('Bridge returned non-JSON. Check the deployment is set to "Anyone" and redeployed.');
  }
