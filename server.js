 ...
 *   GOOGLE_SERVICE_ACCOUNT_JSON   full service-account JSON, pasted as one line (mark SECRET)
 ...
 *
 * Share BOTH sheets with the service account's client_email as Editor.
 * The programming sheet needs Editor because confirming writes the Status row back.
 */
...
const CURRICULUM_ID = process.env.CURRICULUM_SHEET_ID;
const PROGRAMMING_ID = process.env.PROGRAMMING_SHEET_ID;
const CONFIRMERS = (process.env.CONFIRMERS || '')
...
async function warm(force = false) {
  ...
  const [curr, prog] = await Promise.all([loadCurriculum(), loadProgramming()]);
...
async function loadCurriculum() {
  ...
  const notation = keyTab ? parseNotation(await grid(CURRICULUM_ID, keyTab)) : {};
...
function pickWeek(prog, today = new Date()) {
  ...
  // Fallback: nth Monday-week of the month, clamped. Never blocks the coach —
  // it just gets an "unconfirmed" badge in the UI.
  const d = today.getDate();
  const firstDow = new Date(today.getFullYear(), today.getMonth(), 1).getDay();
  const offset = (firstDow + 6) % 7;
  const n = Math.floor((d + offset - 1) / 7);
  return { week: prog.weeks[Math.min(n, prog.weeks.length - 1)], confirmed: false, by: null };
}
...
app.post('/api/week/confirm', async (req, res) => {
  try {
    ...
    await api.spreadsheets.values.update({
      spreadsheetId: PROGRAMMING_ID,
      range: `'${prog.tab}'!${a1(startCol)}${week.statusRow + 1}:${a1(endCol)}${week.statusRow + 1}`,
      valueInputOption: 'RAW',
      requestBody: { values: [cols.map(() => stamp)] },
    });
